...
import random
import time
from core.mall import *
from gameplay.quest import *
from core.customer import *
from core.shop import *
from visuals.decoration import *
...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption("Happy Mall Story")
    ...
    def draw_road_and_environment(self):
        ...
        for i in range(0, SCREEN_WIDTH, 60): pygame.draw.rect(self.screen, YELLOW, (i, 95, 40, 5))
    ...
    def draw_mall_building(self):
        ...
        visible_tiles_y = (SCREEN_HEIGHT - mall_y_start) // TILE_SIZE + 2
    ...
    def draw_ui(self):
        ...
        coin_text = self.font_medium.render(f"Coins: {self.coins}", True, BLACK); pygame.draw.circle(self.screen, YELLOW, (20, 20), 12); pygame.draw.circle(self.screen, ORANGE, (20, 20), 8); pygame.draw.circle(self.screen, BLACK, (20, 20), 12, 2); self.screen.blit(coin_text, (40, 10))
        gem_text = self.font_medium.render(f"Gems: {self.gems}", True, BLACK); points = [(220, 15), (230, 25), (220, 35), (210, 25)]; pygame.draw.polygon(self.screen, BLUE, points); pygame.draw.polygon(self.screen, LIGHT_BLUE, [(215, 20), (220, 25), (215, 30), (210, 25)]); pygame.draw.polygon(self.screen, BLACK, points, 2); self.screen.blit(gem_text, (240, 10))
        level_text = self.font_medium.render(f"Level {self.level}", True, BLACK); self.screen.blit(level_text, (420, 10)); xp_bar_width = 200; xp_progress = (self.xp / self.xp_to_next_level) * xp_bar_width; pygame.draw.rect(self.screen, DARK_GRAY, (420, 40, xp_bar_width, 15), border_radius=7); pygame.draw.rect(self.screen, GREEN, (420, 40, xp_progress, 15), border_radius=7); pygame.draw.rect(self.screen, BLACK, (420, 40, xp_bar_width, 15), 2, border_radius=7); xp_text = self.font_small.render(f"{self.xp}/{self.xp_to_next_level} XP", True, WHITE); self.screen.blit(xp_text, (450, 42))
        
        # Buttons
        self.draw_button("Build Shop", 650, 10, 110, 40, BLUE)
        self.draw_button("Quests", 770, 10, 90, 40, PURPLE)
        self.draw_button("Decorate", 870, 10, 100, 40, GREEN)
        self.draw_button("Mall Info", 980, 10, 100, 40, ORANGE)
        self.draw_button("Expand", 1090, 10, 100, 40, RED)
    ...
    def handle_click(self, pos):
        ...
        any_menu_open = self.show_shop_menu or self.show_decorate_menu or self.show_quest_menu or self.show_mall_info or self.show_expand_menu
        ...
        # UI buttons
        if 650 <= x <= 760 and 10 <= y <= 50: # Build Shop
            self.show_shop_menu = not self.show_shop_menu
            self.show_decorate_menu = False; self.show_quest_menu = False; self.show_mall_info = False; self.show_expand_menu = False
            self.shop_scroll_y = 0 
        elif 770 <= x <= 860 and 10 <= y <= 50: # Quests
            self.show_quest_menu = not self.show_quest_menu
            self.show_shop_menu = False; self.show_decorate_menu = False; self.show_mall_info = False; self.show_expand_menu = False
            self.quest_scroll_y = 0
        elif 870 <= x <= 970 and 10 <= y <= 50: # Decorate
            self.show_decorate_menu = not self.show_decorate_menu
            self.show_shop_menu = False; self.show_quest_menu = False; self.show_mall_info = False; self.show_expand_menu = False
            self.decorate_scroll_y = 0 
        elif 980 <= x <= 1080 and 10 <= y <= 50: # Mall Info
            self.show_mall_info = not self.show_mall_info
            self.show_shop_menu = False; self.show_decorate_menu = False; self.show_quest_menu = False; self.show_expand_menu = False
        elif 1090 <= x <= 1190 and 10 <= y <= 50: # Expand
            self.show_expand_menu = not self.show_expand_menu
            self.show_shop_menu = False; self.show_decorate_menu = False; self.show_quest_menu = False; self.show_mall_info = False
        ...
        # *** PERBAIKAN: Logika klik untuk Menu Expand ***
        if self.show_expand_menu:
            # Re-definisi variabel menu (harus sama dengan draw_expand_menu)
            menu_width = 400
            menu_x = (SCREEN_WIDTH - menu_width) // 2
            menu_y = 150
            menu_center_x = menu_x + (menu_width // 2)

            # Re-definisi y_offset untuk tombol (harus sama dengan draw_expand_menu)
            # y_offset = menu_y + 80
            # y_offset += 40 (current text)
            # y_offset += 60 (next text)
            # y_offset += 50 (cost text)
            button_y = menu_y + 80 + 40 + 60 + 50 # Total = 380
            
            # Koordinat tombol "Upgrade" (sekarang sudah benar)
                    ...
                    if self.coins >= cost:
                        ...
                        for quest in self.quests:
                            if "expand" in quest.description.lower() and not quest.completed:
                                quest.update_progress(1)
                        self.show_expand_menu = False # Tutup menu setelah upgrade
                return
